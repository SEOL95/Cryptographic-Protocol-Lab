...
void RFC7748_C25519(BIG_256_56 r)
{
    int c,lg=0;
    BIG_256_56 t;
    c=CURVE_Cof_I_C25519;
    while (c!=1)
    {
        lg++;
        c/=2;
    }
    int n=8*EGS_C25519-lg+1;
...
int ECP_C25519_IN_RANGE(octet* S)
{
    BIG_256_56 r,s;
    BIG_256_56_rcopy(r, CURVE_Order_C25519);
...
int ECP_C25519_KEY_PAIR_GENERATE(csprng *RNG, octet* S, octet *W)
{
    BIG_256_56 r, s;
    ECP_C25519 G;
    int res = 0;
...
#if CURVETYPE_C25519!=WEIERSTRASS
    RFC7748_C25519(s);                     // For Montgomery or Edwards, apply RFC7748 transformation
#endif
    S->len = EGS_C25519;
    BIG_256_56_toBytes(S->val, s);

    ECP_C25519_clmul(&G, s, r);
    ECP_C25519_toOctet(W, &G, false); /* To use point compression on public keys, change to true */

    return res;
}

/* Validate public key */
int ECP_C25519_PUBLIC_KEY_VALIDATE(octet *W)
{
    BIG_256_56 q, r, k;
    ECP_C25519 WP;
    int valid, nb;
    ...
    valid = ECP_C25519_fromOctet(&WP, W);
    ...
    {   /* Check point is not in wrong group */
        ...
        if (!BIG_256_56_isunity(k)) ECP_C25519_mul(&WP, k);
        if (ECP_C25519_isinf(&WP)) res = ECDH_INVALID_PUBLIC_KEY;
    }
...
int ECP_C25519_SVDP_DH(octet *S, octet *WD, octet *Z, int type)
{
    BIG_256_56 r, s, wx;
    int valid;
    ECP_C25519 W;
    ...
    BIG_256_56_fromBytes(s, S->val);
    valid = ECP_C25519_fromOctet(&W, WD);

    if (!valid) res = ECDH_ERROR;
    if (res == 0)
    {
        BIG_256_56_rcopy(r, CURVE_Order_C25519);
        ECP_C25519_clmul(&W, s, r);
        if (ECP_C25519_isinf(&W)) res = ECDH_ERROR;
        else
        {
#if CURVETYPE_C25519!=MONTGOMERY
            if (type>0)
            {
                if (type==1) ECP_C25519_toOctet(Z,&W,true);
                else ECP_C25519_toOctet(Z,&W,false);
                return res;
            }
            else
                ECP_C25519_get(wx, wx, &W);
#else
            ECP_C25519_get(wx, &W);
#endif
            Z->len = EFS_C25519;
...
int ECP_C25519_SP_DSA(int hlen, csprng *RNG, octet *K, octet *S, octet *F, octet *C, octet *D)
{
    char h[128];
    ...
    int blen = H.len;
    if (H.len > EGS_C25519) blen = EGS_C25519;
    ...
    if (RNG != NULL)
    {
        do
        {
            ...
            ECP_C25519_copy(&V, &G);
            ECP_C25519_clmul(&V, u, r);

            ECP_C25519_get(vx, vx, &V);
...
int ECP_C25519_VP_DSA(int hlen, octet *W, octet *F, octet *C, octet *D)
{
    char h[128];
    octet H = {0, sizeof(h), h};
    ...
    int blen = H.len;
    if (blen > EGS_C25519) blen = EGS_C25519;
    ...
    if (res == 0)
    {
        ...
        {
            ECP_C25519_mul2(&WP, &G, h2, f);
            if (ECP_C25519_isinf(&WP)) res = ECDH_ERROR;
            else
            {
                ECP_C25519_get(d, d, &WP);
                BIG_256_56_mod(d, r);
...
void ECP_C25519_ECIES_ENCRYPT(int hlen, octet *P1, octet *P2, csprng *RNG, octet *W, octet *M, int tlen, octet *V, octet *C, octet *T)
{

    int i, len;
    char z[EFS_C25519], vz[3 * EFS_C25519 + 1], k[2 * AESKEY_C25519], k1[AESKEY_C25519], k2[AESKEY_C25519], l2[8], u[EFS_C25519];
    ...
    if (ECP_C25519_KEY_PAIR_GENERATE(RNG, &U, V) != 0) return;
    if (ECP_C25519_SVDP_DH(&U, W, &Z, 0) != 0) return;

    OCT_copy(&VZ, V);
    OCT_joctet(&VZ, &Z);

    KDF2(MC_SHA2, hlen, &K, 2 * AESKEY_C25519, &VZ, P1);

    K1.len = K2.len = AESKEY_C25519;
    for (i = 0; i < AESKEY_C25519; i++)
    {
        K1.val[i] = K.val[i];
        K2.val[i] = K.val[AESKEY_C25519 + i];
    }
...
int ECP_C25519_ECIES_DECRYPT(int hlen, octet *P1, octet *P2, octet *V, octet *C, octet *T, octet *U, octet *M)
{

    int i, len;
    char z[EFS_C25519], vz[3 * EFS_C25519 + 1], k[2 * AESKEY_C25519], k1[AESKEY_C25519], k2[AESKEY_C25519], l2[8], tag[32];
    octet Z = {0, sizeof(z), z};
    octet VZ = {0, sizeof(vz), vz};
    octet K = {0, sizeof(k), k};
    octet K1 = {0, sizeof(k1), k1};
    ...
    if (ECP_C25519_SVDP_DH(U, V, &Z, 0) != 0) return 0;
