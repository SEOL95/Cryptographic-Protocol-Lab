...
int ECP_C25519_isinf(ECP_C25519 *P)
{

#if CURVETYPE_C25519==EDWARDS
    return (FP_F25519_iszilch(&(P->x)) && FP_F25519_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_C25519==WEIERSTRASS
...
static void ECP_C25519_cswap(ECP_C25519 *P, ECP_C25519 *Q, int d)
{
...
static void ECP_C25519_cmove(ECP_C25519 *P, ECP_C25519 *Q, int d)
{
...
#endif // CURVETYPE_C25519!=MONTGOMERY

#if CURVETYPE_C25519!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_C25519_select(ECP_C25519 *P, ECP_C25519 W[], sign32 b)
{
    ECP_C25519 MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
    ...
    ECP_C25519_cmove(P, &W[0], teq(babs, 0)); // conditional move
    ECP_C25519_cmove(P, &W[1], teq(babs, 1));
    ECP_C25519_cmove(P, &W[2], teq(babs, 2));
    ECP_C25519_cmove(P, &W[3], teq(babs, 3));
    ECP_C25519_cmove(P, &W[4], teq(babs, 4));
    ECP_C25519_cmove(P, &W[5], teq(babs, 5));
    ECP_C25519_cmove(P, &W[6], teq(babs, 6));
    ECP_C25519_cmove(P, &W[7], teq(babs, 7));

    ECP_C25519_copy(&MP, P);
    ECP_C25519_neg(&MP);  // minus P
    ECP_C25519_cmove(P, &MP, (int)(m & 1));
}
#endif

/* Test P == Q */
/* SU=168 */
int ECP_C25519_equals(ECP_C25519 *P, ECP_C25519 *Q)
{
    FP_F25519 a, b;
...
/* Set P=Q */
/* SU=16 */
void ECP_C25519_copy(ECP_C25519 *P, ECP_C25519 *Q)
{
...
#if CURVETYPE_C25519!=MONTGOMERY
/* SU=8 */
void ECP_C25519_neg(ECP_C25519 *P)
{
#if CURVETYPE_C25519==WEIERSTRASS
...
/* Set P=O */
void ECP_C25519_inf(ECP_C25519 *P)
{
    FP_F25519_zero(&(P->x));
#if CURVETYPE_C25519!=MONTGOMERY
    FP_F25519_one(&(P->y));
...
/* SU=56 */
void ECP_C25519_rhs(FP_F25519 *v, FP_F25519 *x)
{
#if CURVETYPE_C25519==WEIERSTRASS
    /* x^3+Ax+B */
    FP_F25519 t;
    ...
    FP_F25519_mul(&t, &t, x);
        ...
        FP_F25519_neg(v, x);
        FP_F25519_norm(v);
        FP_F25519_imul(v, v, -CURVE_A_C25519);
        FP_F25519_norm(v);
        FP_F25519_add(v, &t, v);
#else 
        FP_F25519_copy(v, &t);
    ...
    FP_F25519_rcopy(&t, CURVE_B_C25519);
    ...
    FP_F25519_copy(v, x);
    FP_F25519_imul(&x2, &x2, CURVE_A_C25519);
...
int ECP_C25519_set(ECP_C25519 *P, BIG_256_56 x)
{
    //BIG_256_56 m, b;
    FP_F25519 rhs;
...
int ECP_C25519_get(BIG_256_56 x, ECP_C25519 *P)
{
    ECP_C25519 W;
    ECP_C25519_copy(&W, P);
    ECP_C25519_affine(&W);
    if (ECP_C25519_isinf(&W)) return -1;
...
/* SU=16 */
int ECP_C25519_get(BIG_256_56 x, BIG_256_56 y, ECP_C25519 *P)
{
    ECP_C25519 W;
    ECP_C25519_copy(&W, P);
    ECP_C25519_affine(&W);
...
/* Set P=(x,{y}) */
/* SU=96 */
int ECP_C25519_set(ECP_C25519 *P, BIG_256_56 x, BIG_256_56 y)
{
    FP_F25519 rhs, y2;
...
/* SU=136 */
int ECP_C25519_setx(ECP_C25519 *P, BIG_256_56 x, int s)
{
    FP_F25519 rhs,hint;
    FP_F25519_nres(&rhs, x);
...
void ECP_C25519_cfp(ECP_C25519 *P)
{   /* multiply point by curves cofactor */
    BIG_256_56 c;
    int cf = CURVE_Cof_I_C25519;
    if (cf == 1) return;
    if (cf == 4)
    {
        ECP_C25519_dbl(P);
    ...
    BIG_256_56_rcopy(c, CURVE_Cof_C25519);
    ECP_C25519_mul(P, c);
    return;
}
...
/*
void ECP_C25519_hap2point(ECP_C25519 *P,BIG_256_56 h)
{
    BIG_256_56 x;
    BIG_256_56_copy(x,h);
	...
	{
#if CURVETYPE_C25519!=MONTGOMERY
		ECP_C25519_setx(P,x,0);
#else
		ECP_C25519_set(P,x);
#endif
		BIG_256_56_inc(x,1); BIG_256_56_norm(x);
		if (!ECP_C25519_isinf(P)) break;
	}
}
*/
/* Constant time Map to Point */
void ECP_C25519_map2point(ECP_C25519 *P,FP_F25519 *h)
{
#if CURVETYPE_C25519==MONTGOMERY
// Elligator 2
    int qres;
    ...
    //BIG_256_56_zero(a); BIG_256_56_inc(a,CURVE_A_C25519); BIG_256_56_norm(a); FP_F25519_nres(&A,a);
    FP_F25519_from_int(&A,CURVE_A_C25519);
    FP_F25519_copy(&t,h);
...
// Figure out the Montgomery curve parameters
    ...
    FP_F25519_rcopy(&B,CURVE_B_C25519);
#if CURVE_A_C25519 ==  1
...
// Figure out RHS of Montgomery curve in rational form gx1/d^3
    ...
    FP_F25519_rcopy(&w,CURVE_HTPC_C25519);   // qnr^C3  
...
// Map to point on isogenous curve
    int i,k,isox,isoy,iso=HTC_ISO_C25519;
    FP_F25519 xnum,xden,ynum,yden;
    BIG_256_56 z;
    FP_F25519_rcopy(&A,CURVE_Ad_C25519);
    FP_F25519_rcopy(&B,CURVE_Bd_C25519);
#else
    FP_F25519_from_int(&A,CURVE_A_C25519);
    FP_F25519_rcopy(&B,CURVE_B_C25519);
#endif
...
#if CURVE_A_C25519 != 0 || HTC_ISO_C25519 != 0
...
// x^3+Ad^2x+Bd^3
        ...
        FP_F25519_rcopy(&X1,CURVE_HTPC_C25519);     
...
// xnum
        FP_F25519_rcopy(&xnum,PC_C25519[k++]);
        for (i=0;i<isox;i++)
        {
            FP_F25519_mul(&xnum,&xnum,&X2); 
            FP_F25519_rcopy(&w,PC_C25519[k++]);
...
// ynum
        FP_F25519_rcopy(&ynum,PC_C25519[k++]);
        for (i=0;i<isoy;i++)
        {
...
/* Map octet to point */
/*
void ECP_C25519_mapit(ECP_C25519 *P, octet *W)
{
    BIG_256_56 q, x;
    DBIG_256_56 dx;
    ...
    ECP_C25519_hap2point(P,x);
    ECP_C25519_cfp(P);
}
*/
/* Convert P to Affine, from (x,y,z) to (x,y) */
/* SU=160 */
void ECP_C25519_affine(ECP_C25519 *P)
{
    FP_F25519 one, iz;
    if (ECP_C25519_isinf(P)) return;
    FP_F25519_one(&one);
...
#if CURVETYPE_C25519==EDWARDS || CURVETYPE_C25519==WEIERSTRASS
...
/* SU=120 */
void ECP_C25519_outputxyz(ECP_C25519 *P)
{
    BIG_256_56 x, z;
    if (ECP_C25519_isinf(P))
    {
        printf("Infinity\n");
        return;
    }
...
/* SU=16 */
/* Output point P */
void ECP_C25519_output(ECP_C25519 *P)
{
    BIG_256_56 x;
    if (ECP_C25519_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void ECP_C25519_rawoutput(ECP_C25519 *P)
{
    BIG_256_56 x, z;
...
void ECP_C25519_toOctet(octet *W, ECP_C25519 *P, bool compress)
{
#if CURVETYPE_C25519==MONTGOMERY
    BIG_256_56 x;
    ECP_C25519_get(x, P);
    ...
    bool alt=false;
    ECP_C25519_affine(P);
    ECP_C25519_get(x, y, P);

#if (MBITS-1)%8 <= 4
#ifdef ALLOW_ALT_COMPRESS_C25519
    alt=true;
#endif
#endif
    if (alt)
...
int ECP_C25519_fromOctet(ECP_C25519 *P, octet *W)
{
#if CURVETYPE_C25519==MONTGOMERY
    BIG_256_56 x;
    BIG_256_56_fromBytes(x, &(W->val[0]));
    if (ECP_C25519_set(P, x)) return 1;
    return 0;
#else
    BIG_256_56 x, y;
    bool alt=false;
    ...
    if (alt)
    {
        ...
        if ((typ&0x80)==0)
        {
            ...
            if (ECP_C25519_set(P, x, y)) return 1;
            return 0;
        } else {
            if (!ECP_C25519_setx(P,x,0)) return 0;
            sgn=(typ&0x20)>>5;
            cmp=FP_F25519_islarger(&(P->y));
            if ((sgn==1 && cmp!=1) || (sgn==0 && cmp==1)) ECP_C25519_neg(P);
            return 1;
        }
        ...
        {
            if (ECP_C25519_setx(P, x, typ & 1)) return 1;
        }
    }
    return 0;
#endif
...
/* Set P=2P */
/* SU=272 */
void ECP_C25519_dbl(ECP_C25519 *P)
{
#if CURVETYPE_C25519==WEIERSTRASS
    FP_F25519 t0, t1, t2, t3, x3, y3, z3, b;
        ...
        FP_F25519_imul(&t2, &t2, 3 * CURVE_B_I_C25519);   //t2.imul(3*ROM.CURVE_B_I);
        ...
        if (CURVE_B_I_C25519 == 0)                 //if (ROM.CURVE_B_I==0)
            FP_F25519_rcopy(&b, CURVE_B_C25519);      //b.copy(new FP(new BIG(ROM.CURVE_B)));
        ...
        if (CURVE_B_I_C25519 == 0)                     //if (ROM.CURVE_B_I==0)
            ...
            FP_F25519_imul(&y3, &t2, CURVE_B_I_C25519); //y3.imul(ROM.CURVE_B_I);
        ...
        if (CURVE_B_I_C25519 == 0)                 //if (ROM.CURVE_B_I==0)
            ...
            FP_F25519_imul(&z3, &z3, CURVE_B_I_C25519); //z3.imul(ROM.CURVE_B_I);
    ...
    FP_F25519_imul(&A, &C, (CURVE_A_C25519 + 2) / 4); //A.imul((ROM.CURVE_A+2)/4);
...
void ECP_C25519_add(ECP_C25519 *P, ECP_C25519 *Q, ECP_C25519 *W)
{
...
/* Set P+=Q */
/* SU=248 */
void ECP_C25519_add(ECP_C25519 *P, ECP_C25519 *Q)
{
#if CURVETYPE_C25519==WEIERSTRASS
    ...
    FP_F25519 t0, t1, t2, t3, t4, x3, y3, z3, b;
        ...
        b3 = 3 * CURVE_B_I_C25519;             //int b=3*ROM.CURVE_B_I;
        ...
        if (CURVE_B_I_C25519 == 0)             //if (ROM.CURVE_B_I==0)
            ...
            FP_F25519_imul(&z3, &t2, CURVE_B_I_C25519); //z3.imul(ROM.CURVE_B_I);
        ...
        if (CURVE_B_I_C25519 == 0)             //if (ROM.CURVE_B_I==0)
            ...
            FP_F25519_imul(&y3, &y3, CURVE_B_I_C25519); //y3.imul(ROM.CURVE_B_I);
    ...
    if (CURVE_B_I_C25519 == 0)         //if (ROM.CURVE_B_I==0)
    {
        FP_F25519_rcopy(&b, CURVE_B_C25519);  //FP b=new FP(new BIG(ROM.CURVE_B));
        FP_F25519_mul(&E, &E, &b);         //E.mul(b);
    }
    else
        FP_F25519_imul(&E, &E, CURVE_B_I_C25519); //E.imul(ROM.CURVE_B_I);
...
/* Set P-=Q */
/* SU=16 */
void  ECP_C25519_sub(ECP_C25519 *P, ECP_C25519 *Q)
{
    ECP_C25519 NQ;
    ECP_C25519_copy(&NQ, Q);
    ECP_C25519_neg(&NQ);
    ECP_C25519_add(P, &NQ);
}
...
void ECP_C25519_pinmul(ECP_C25519 *P, int e, int bts)
{
    int i, b;
    ECP_C25519 R0, R1;
    ...
    ECP_C25519_affine(P);
    ECP_C25519_inf(&R0);
    ECP_C25519_copy(&R1, P);

    for (i = bts - 1; i >= 0; i--)
    {
        b = (e >> i) & 1;
        ECP_C25519_copy(P, &R1);
        ECP_C25519_add(P, &R0);
        ECP_C25519_cswap(&R0, &R1, b);
        ECP_C25519_copy(&R1, P);
        ECP_C25519_dbl(&R0);
        ECP_C25519_cswap(&R0, &R1, b);
    }
    ECP_C25519_copy(P, &R0);
}
#endif
...
void ECP_C25519_mul(ECP_C25519 *P,BIG_256_56 e)
{
    ECP_C25519_clmul(P,e,e);
}

// .. but this one does not (typically set maxe=r)
// Set P=e*P 
void ECP_C25519_clmul(ECP_C25519 *P, BIG_256_56 e, BIG_256_56 maxe)
{
    int max;
    BIG_256_56 cm;
#if CURVETYPE_C25519==MONTGOMERY
    ...
    ECP_C25519_copy(&W[0], P);

    for (i = 1; i < 8; i++)
    {
        ECP_C25519_copy(&W[i], &W[i - 1]);
        ECP_C25519_add(&W[i], &Q);
    }
...
//printf("W[1]= ");ECP_output(&W[1]); printf("\n");
    ...
    //ECP_C25519_copy(P, &W[(w[nb] - 1) / 2]);
    ECP_C25519_select(P, W, w[i]);
    for (i = nb - 1; i >= 0; i--)
    {
        ECP_C25519_select(&Q, W, w[i]);
        ECP_C25519_dbl(P);
        ECP_C25519_dbl(P);
        ECP_C25519_dbl(P);
        ECP_C25519_dbl(P);
    ...
    ECP_C25519_sub(P, &C); /* apply correction */
#endif
}
...
void ECP_C25519_muln(ECP_C25519 *P,int n,ECP_C25519 X[],BIG_256_56 e[])
{
    int i,j,k,nb;
    BIG_256_56 t,mt;
    ECP_C25519 S,R,B[16];
    ...
    { // Pippenger's algorithm
        ...
        for (j=0;j<n;j++)
        {
            ...
            ECP_C25519_add(&B[k],&X[j]);
        }
        ECP_C25519_inf(&R); ECP_C25519_inf(&S);
        for (j=15;j>=1;j--)
        {
            ECP_C25519_add(&R,&B[j]);
            ECP_C25519_add(&S,&R);
        }
        for (j=0;j<4;j++)
            ECP_C25519_dbl(P);
...
void ECP_C25519_mul2(ECP_C25519 *P, ECP_C25519 *Q, BIG_256_56 e, BIG_256_56 f)
{
    BIG_256_56 te, tf, mt;
    ECP_C25519 S, T, W[8], C;
    ...
    /* precompute table */
    ECP_C25519_copy(&W[1], P);
    ECP_C25519_sub(&W[1], Q); /* P+Q */
    ECP_C25519_copy(&W[2], P);
    ECP_C25519_add(&W[2], Q); /* P-Q */
    ECP_C25519_copy(&S, Q);
    ECP_C25519_dbl(&S);  /* S=2Q */
    ECP_C25519_copy(&W[0], &W[1]);
    ECP_C25519_sub(&W[0], &S);
    ECP_C25519_copy(&W[3], &W[2]);
    ECP_C25519_add(&W[3], &S);
    ECP_C25519_copy(&T, P);
    ECP_C25519_dbl(&T); /* T=2P */
    ECP_C25519_copy(&W[5], &W[1]);
    ECP_C25519_add(&W[5], &T);
    ECP_C25519_copy(&W[6], &W[2]);
    ECP_C25519_add(&W[6], &T);
    ECP_C25519_copy(&W[4], &W[5]);
    ECP_C25519_sub(&W[4], &S);
    ECP_C25519_copy(&W[7], &W[6]);
    ECP_C25519_add(&W[7], &S);
...
int ECP_C25519_generator(ECP_C25519 *G)
{
    BIG_256_56 x, y;
    BIG_256_56_rcopy(x, CURVE_Gx_C25519);
#if CURVETYPE_C25519!=MONTGOMERY
    BIG_256_56_rcopy(y, CURVE_Gy_C25519);
    return ECP_C25519_set(G, x, y);
#else
    return ECP_C25519_set(G, x);
#endif
}
...
#if CURVETYPE_C25519!=MONTGOMERY
    ECP_C25519_set(&G, x, y);
#else
    ECP_C25519_set(&G, x);
#endif
    if (ECP_C25519_isinf(&G)) printf("Failed to set - point not on curve\n");
